...
# Agent for publications

class PublicationsAgent(BaseAgent):
    def __init__(self):
        # TODO: Exchange the dummy parts
        with open('data/dummy_data.txt', 'r', encoding='utf-8') as f:
            self.data = f.read()

        #self.index = faiss.read_index('data/publications.faiss')
        #with open('data/publications_docs.txt', 'r', encoding='utf-8') as f:
    ...
    def execute(self, query):
        ...
        ## TODO: routine to output the publication as pdf
        #context = "\n".join(relevant_docs)
        #return context